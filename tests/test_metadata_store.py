...
def test_sidecar_uses_dataset_name(tmp_path, monkeypatch):
    """The sidecar filename is the dataset name, even when the data file differs."""
    data = tmp_path / "data"
    data.mkdir(exist_ok=True)
    monkeypatch.setattr(metadata_store, "get_sidecar_dir", lambda: data)

    seed = {"volume": "MitoHardLiver", "dataset": "MitoHardLiver"}
    sidecar = metadata_store.write_metadata_sidecar(seed)
    assert sidecar.endswith("mitohardliver.metadata.json")
